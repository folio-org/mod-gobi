...
public class Mapper {
              ...
              .thenAccept(o -> {
                ...
                //Map Receipt dates based on the order format type
                  ...
                  .ifPresent(orderformat -> orderformat.resolve(doc)
                        .thenAccept(format->{
                            if(((String) format).equalsIgnoreCase(CompositePoLine.OrderFormat.ELECTRONIC_RESOURCE.toString())){
                              ...
                              .ifPresent(expectedReceiptInterval -> physical.setExpectedReceiptDate(Date.from(dt.plusDays(expectedReceiptInterval)
  ...
  private void mapPurchaseOrderLine(List<CompletableFuture<?>> futures,CompositePoLine pol, Document doc) {
      ...
      Optional.ofNullable(mappings.get(Mapping.Field.COLLECTION)).ifPresent(field -> futures.add(field.resolve(doc)
  ...
  private void mapLocation(List<CompletableFuture<?>> futures, Location location, Document doc) {
    ...
    // A GOBI order can only be one of the below type per order, hence the total quantity will be the same
      Optional.ofNullable(mappings.get(Mapping.Field.QUANTITY_ORDERED_ELECTRONIC))
          .ifPresent(field -> futures.add(field.resolve(doc)
          .thenAccept(o -> {
  ...
  private void mapVendorDetail(List<CompletableFuture<?>> futures, VendorDetail vendorDetail, Document doc) {
      ...
      Optional.ofNullable(mappings.get(Mapping.Field.VENDOR_REF_NO)
          ...
          .thenAccept(o -> {
            ...
            Optional.ofNullable(mappings.get(Mapping.Field.VENDOR_REF_NO_TYPE)
                ...
                .thenAccept(numberType -> vendorDetail.setRefNumberType(
                    VendorDetail.RefNumberType.fromValue((String) numberType)))
                .exceptionally(Mapper::logException)));
          })
          .exceptionally(Mapper::logException)));
      ...
      .ifPresent(field -> futures.add(field
               .resolve(doc)
